...
exports.IMAGE_COMPRESSION_SERVICE = {
  async CompressImage(image, percentage) {
  ...
  async bulkCompressImages(images, percentage) {
    const compressedImages = [];
    for (let i = 0; i < images.length; i++) {
      compressedImages[i] = await this.CompressImage(images[i], percentage);
    }
